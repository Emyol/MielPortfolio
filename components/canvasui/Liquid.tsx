...
void main () {
  ...
  if (uHasContent < 0.5) {
    ...
    vec2 flow = fluid.xy;
    float ridges = smoothstep(0.02, 1.05, mag * uIntensity * 0.42);
    float aniso = abs(dot(normalize(flow + 1e-4), vec2(0.72, 0.28)));
    float highlight = pow(ridges, 1.25) * (0.38 + 0.62 * aniso);
    vec3 metal = mix(vec3(0.06), uColor, 0.32 + ridges * 0.68);
    metal += uColor * highlight * 1.05;
    float overlay = mix(0.12, 0.84, ridges);
    outColor = vec4(toSrgb(clamp(metal, 0.0, 1.0)) * overlay, overlay);
    return;
  }
...
export function createLiquid(
  ...
  function seedAmbient(now: number) {
    const a = now * 0.00036;
    const b = now * 0.00022;
    queued.push([
      0.5 + Math.cos(a) * 0.36,
      0.46 + Math.sin(a * 0.71) * 0.3,
      Math.cos(a * 1.35) * 42,
      Math.sin(b * 1.12) * 38,
    ]);
    queued.push([
      0.38 + Math.sin(b) * 0.3,
      0.64 + Math.cos(a * 0.88) * 0.24,
      Math.sin(a * 0.9) * -28,
      Math.cos(b * 1.4) * 26,
    ]);
  }
  ...
  function onMotionChange() {
    ...
    output.style.opacity = reducedMotion ? "0" : "1";
    if (reducedMotion) {
...
export function Liquid({
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      const liquid = instanceRef.current;
      if (!liquid) return;
      liquid.splat(0.12, 0.38, 140, 36);
      liquid.splat(0.42, 0.58, -120, 48);
      liquid.splat(0.78, 0.28, 95, -40);
      liquid.splat(0.32, 0.82, -40, 110);
      liquid.splat(0.9, 0.68, -80, 28);
      liquid.splat(0.55, 0.18, 30, 90);
    });
    return () => {
    ...
    <div className={className} style={{ position: "relative", ...style }}>
      ...
      <canvas
        ...
        style={{
          ...
          height: "100%",
          pointerEvents: "none",
          mixBlendMode: native ? "normal" : "screen",
        }}
      />
    </div>
  );
